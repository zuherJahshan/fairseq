...
print("Loading the audiofiles...")
audiofiles = glob.glob('/workspace/fairseq/data/train_val/**/*.flac', recursive=True)
...
for audiofiles, mels in tqdm(loader):
    ...
    for (audiofile, result) in zip(audiofiles, results):
        ...
        result_file = audiofile.replace('.flac', '.txt').replace('train_val', 'results')

...
class DinosrAudioConfig(Wav2Vec2Config):
    ...
    model_architecture: str = field(default="transformer", metadata={"help": "model type"}) # could be transformer or Mamba


def create_mamba_model(cfg: DinosrAudioConfig):
    # convert DInosaurAudioConfig to MambaConfig
    print("==================> Creating Mamba model <==================")
    mamba_cfg = MambaConfig()
    print(f"the cfg encoder embed dim is {cfg.encoder_embed_dim}")
    mamba_cfg.vocab_size = cfg.encoder_embed_dim
    mamba_cfg.d_model = cfg.encoder_embed_dim 
    mamba_cfg.n_layer = cfg.encoder_layers
    return MambaLMHeadModel(mamba_cfg)
...
def get_model(cfg: DinosrAudioConfig):
    if cfg.model_architecture == "transformer":
        return TransformerEncoder(cfg)
    else: # should return Mamba here
        return create_mamba_model(cfg)
...
class DinosrModel(BaseFairseqModel):
    ...
    def make_ema_teacher(self):
        ...
        skip_keys = set()
        if self.cfg.ema_layers_only and self.cfg.model_architecture == "transformer":
        ...
        with torch.no_grad():
            ...
            if self.cfg.instance_norm_target_layer or self.cfg.batch_norm_target_layer:
                target_layer_results = [
                    rearrange(tl, 't b c -> b c t') for tl in target_layer_results
                ]
                permuted = True
